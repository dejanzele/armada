...
SELECT run_id, job_id, created, job_set, executor, node, cancelled, running, succeeded, failed, returned, run_attempted, serial, last_modified, leased_timestamp, pending_timestamp, running_timestamp, terminated_timestamp, scheduled_at_priority, preempted, pending, preempted_timestamp, pod_requirements_overlay, preempt_requested, queue, pool, terminated, run_index FROM runs WHERE serial > $1 AND job_id = ANY($3::text[]) ORDER BY serial LIMIT $2
`
...
WHERE jr.executor = $1
  ...
  AND ($3::text[] IS NULL OR cardinality($3::text[]) = 0 OR jr.pool = ANY($3::text[]))
...
WHERE jr.queue = ANY($1::text[])
  ...
  AND jr.terminated = false
  AND jr.preempted = false
  AND ($2::text[] IS NULL OR cardinality($2::text[]) = 0 OR jr.pool = ANY($2::text[]))
`
...
SELECT run_id, job_id, created, job_set, executor, node, cancelled, running, succeeded, failed, returned, run_attempted, serial, last_modified, leased_timestamp, pending_timestamp, running_timestamp, terminated_timestamp, scheduled_at_priority, preempted, pending, preempted_timestamp, pod_requirements_overlay, preempt_requested, queue, pool, terminated, run_index FROM runs WHERE serial > $1 ORDER BY serial LIMIT $2
`
...
SELECT run_id, job_id, created, job_set, executor, node, cancelled, running, succeeded, failed, returned, run_attempted, serial, last_modified, leased_timestamp, pending_timestamp, running_timestamp, terminated_timestamp, scheduled_at_priority, preempted, pending, preempted_timestamp, pod_requirements_overlay, preempt_requested, queue, pool, terminated, run_index FROM runs WHERE serial > $1 AND job_id = ANY($2::text[]) ORDER BY serial
`
...
WHERE j.queue = ANY($1::text[])
  AND j.queued = true
  AND ($2::text[] IS NULL OR cardinality($2::text[]) = 0 OR j.pools && $2::text[])
`
